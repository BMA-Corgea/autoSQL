...
BEGIN
  ...
  ELSIF jsonb_typeof(j) = 'string' THEN
    ...
    IF t !~ '^[+-]?([0-9]+\.[0-9]*|\.[0-9]+|[0-9]+)([eE][+-]?[0-9]+)?$' THEN
      -- T-6 variant C: MATCH the Python evaluator instead of refusing, and pay for it
      -- only when the ASCII gate has already failed.
      --
      -- Python's float() accepts any Unicode decimal digit by its numeric value and
      -- strips its full 29-code-point isspace set; SQL's ASCII gate above accepts
      -- neither, and used to return NULL -- a SILENT WRONG NUMBER (T-3 M1 and D10).
      -- Mapping the 670 non-ASCII Nd code points onto '0'-'9' makes the two engines
      -- agree outright, with no refusal and so no collateral loss of expressions that
      -- were already answering correctly.
      --
      -- The nesting is load-bearing, NOT tidiness: translate() over a 670-character
      -- from-set costs about 12 microseconds a call, and hoisting it above the ASCII
      -- gate makes every ordinary numeric string pay it. Measured on this machine:
      -- 852 ms unhoisted vs 4553 ms hoisted, per 300k ASCII coercions (5.3x). Leave
      -- it inside the failure branch.
