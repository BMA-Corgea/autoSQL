...
**ANSWER:** **Install the automatic hook.** Applied to `.claude/settings.json` as a `Stop` hook running the two-leg drain (`notify.mjs process` then `ops/notify-telegram.sh`). **Proven, not assumed:** the drain leg ran clean (`{paged:0,alerts:0,feed:0}` — correct, nothing was waiting), and `ops/notify-telegram.sh --test` sent one ping that reached his phone. The running-commentary feed was NOT switched on — he picked the hook alone. Note: the hook registers for sessions started after this edit.
