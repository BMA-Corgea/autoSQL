...
**38 items. 5 block tomorrow.**

Most are decisions I took on your behalf while you were away — each derived from something you
already said, each one line to overturn. Write on the `ANSWER:` line; blank takes the stated default.

**Start with Section A.** It was the authority for about twenty-one of the others.
...
- [ ] **It covered everything I was asked** — All rulings taken under it stand as written; nothing changes.
...
- [ ] **Right — tick is GIMS, note is the demo** — Nothing changes; the demo proceeds and GIMS stays untouched.
...
- [ ] **Approve as drawn** — The build copies the mock exactly; nothing changes.
...
- [ ] **Commit and push it as it stands** — Tomorrow starts from a clean, matching copy on both machines, with the spec folder move and the ticket advance recorded as done.
...
- [ ] *Take your stated default* — Stands. Only reporting changes if you reverse it; nothing needs re-running.
...
- [ ] *Take your stated default* — The three bars stand. Cheap either way — the run reports raw milliseconds, so you can redraw any line afterwards without re-running anything.
...
- [ ] *Take your stated default* — 25 / 25 / 9 repetitions, with worst-of-9 standing in for the percentile at a million rows.
...
- [ ] *Take your stated default* — Operation 9 ships, comparing everything except the timestamp, with no control for it.
...
- [ ] *Take your stated default* — Runtime check, with the refusal happening per row rather than per query.
...
- [ ] *Take your stated default* — The two-layer design gets built.
...
- [ ] *Take your stated default* — The strict check ships and is shown firing on screen.
...
- [ ] *Take your stated default* — 8,400 heartbeats + 2,000 samples + 10 edge cases, dated 14–20 August 2026.
...
- [ ] *Take your stated default* — The demo's own comparator, tiebreaking on the record key.
...
- [ ] *Take your stated default* — 6-decimal half-up rounding, a 3-row trailing average with short-window divisors, hour/day buckets in UTC, and no on-screen control for any of it.
...
- [ ] *Take your stated default* — The run scores exactly as ruled.
...
- [ ] *Take your stated default* — Both runs stop for you, and both prove their own checks before quoting numbers.
...
- [ ] *Take your stated default* — Byte-identical copies with checksum drift checks.
...
- [ ] *Take your stated default* — The mock is built as drawn. The design document also carries five look questions it says only ten minutes of driving the real screen can settle.
...
- [ ] *Take your stated default* — All stand as written.
...
- [ ] *Take your stated default* — 90 minutes and Denver time stand, and every hours figure this project reports from now on is built on them.
...
- [ ] *Take your stated default* — The script-level refusal stands as the only protection, and the live container keeps running alongside every future run on this machine.
...
- [ ] *Take your stated default* — The write-up stands as it is, and anyone reading it later — including him — sees a cost figure roughly two hundred times too large presented as measured.
...
- [ ] **Hand it to GIMS** — It stays GitHub issue #9 in the GIMS project and gets scheduled over there, not here.
...
- [ ] **Builder decides** — Whoever writes the code rules each one and records how, which is what the punch list already instructs.
...
- [ ] **Install the automatic hook** — The drafted three-line config goes into this repo's settings; a ping fires whenever a session ends its turn.
...
- [ ] **Correctness run alongside the demo, timing run alone later** — The correctness run does not need an idle machine the way the timing run does, so only the timing run needs a booked window.
...
- [ ] **File it** — One issue covering all three, opened on the author's private repo under his GitHub account; he gets the link.
...
- [ ] **Fix it with the full-process switch** — Running T-2's remaining steps as dispatched workers makes the split take effect for free. One check proves it: after the next step, the worker.started line in the event log should name a model that is not Opus.
...
- [ ] **Make acceptance strict, leave the rest** — Smallest change: acceptance is the one checkpoint your own Q34 answer relies on as the safety net. `node tracker.mjs override T-2 --gate accept --policy human:strict`, or set it shop-wide in .autodev/data/gates-policy.json.
...
- [ ] **Keep the bargain, but make it real** — Set acceptance to `human:strict` so the demo genuinely cannot ship until you have driven it yourself. No test runner needed; costs you one real stop.
...
- [ ] **Make it part of the process** — Add "refresh CURRENT-WORK" to whatever must happen before a session ends, so a page that claims to be updated at every handoff actually is.
...
- [ ] **Skip it — spot-check as you go** — You accept them all by default and challenge individual ones as they surface during the build. Cheapest now, and the derivations stay findable in each document.
