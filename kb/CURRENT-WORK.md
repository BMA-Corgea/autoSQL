...
never when a ticket *arrives* at one. `gate_waiting` has now fired **12** times
(`spec_ready` 7, `accept` 6) — *corrected 2026-09-08; this line used to say 0, which was true when
written on 2026-08-22 and stopped being true on 2026-09-01.* But **`sp_decide` has fired 0 times in
...
decides. **As of T-18 the packet is written for you**: `ops/gate-ping.mjs` runs in the `Stop` hook and
announces anything parked at an uncleared human gate. It does **not** restore the ledger event or the
passport stamp — it never writes ticket state — so the record is still incomplete and the packet says
so. Filing upstream stays his call (`.autodev/notes/upstream-bugs.md` Defect 4).
- **T-18** — waiting at spec_ready on human:owner since 2026-09-08
